...
$('.q3-second textarea, .q3-first textarea, #question-1-textarea-2, #question-1-textarea-1, #question-2-textarea-2').on('click input blur focus change keyup keydown', function(){
	var preg = $(this).val();
	// var preg = $(this).val().replace(/^\.|[^-?\d\.]|\.(?=.*\.)|^0+(?=\d)/g, '');	
	// preg = preg.replace(/\$[0-9,]+(?:\.\d\d)?/g);
	// preg = preg.replace(/[a-zа-яё]/gi, '');
	// preg = preg.replace(/[!?@#$%^&*();:№"'_~/\+\-]/gi, '');
...
$('.question-1-radio').change(function(event) {
	...
	if($('#question-1-radio-1').is(':checked')){
		$('.peregorodki').show();
	}else{
		$('.peregorodki').hide();
	}
...
function calcResult(){
	...
	let zhilSquareCost = 3900;
	let zhilSquareRes = 0;
	let bathroomsSquareRes = 0;
	let bathroomsSquareCost = 60000;
	...
	if(premisesSquare >= 10){
		premisesSquareCost = 4680;
	}else{
		premisesSquareCost = 7488;
	}	
...
// 	let bathroomSquareCost = 20000;
	
	var bathroomSquareCost = 30000;
      if ($('#question-1-radio-1').prop('checked')) {
        var bathroomSquareCost = 20000;
      }
...
// 		console.log('bathroomSquare = ' + bathroomSquare);
	...
	$("[name='Итоговая стоимость']").val(allResult);
...
// $('.js-quiz__question--1 textarea, .js-quiz__question--2 textarea, .js-quiz__question--3 textarea').on('click input blur focus change', function(){
// 	if($(this).val() > 0){
